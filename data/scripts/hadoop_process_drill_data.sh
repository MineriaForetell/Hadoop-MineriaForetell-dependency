...
hadoop fs -rm -r -f /FilesDrillingData && 
hadoop fs -put $(eval echo ~$USER)/data/drillfiles/ /FilesDrillingData &&


echo "Data moved to hdfs. Now loading in hive" &&
...
hadoop fs -rm -r -f /MiningConvertedData &&
...
rm -rf $(eval echo ~$USER)/data/processed/MiningConvertedData/* &&

hadoop fs -get /MiningConvertedData $(eval echo ~$USER)/data/processed/
